...
export const LoveCalculator: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === 'Enter' && !isCalculating && result === null) {
        if (document.activeElement === maleInputRef.current) {
          femaleInputRef.current?.focus();
        } else if (document.activeElement === femaleInputRef.current && maleName.trim() && femaleName.trim()) {
          handleCalculate();
        }
  ...
  }, [maleName, femaleName, isCalculating, result]);
